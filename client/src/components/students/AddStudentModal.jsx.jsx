...
export function AddStudentModal({
  ...
  // Arrays seguros para evitar null/undefined
  const safeExistingStudents = Array.isArray(existingStudents)
    ? existingStudents.filter(Boolean)
    : [];
  const safeAllStudents = Array.isArray(allStudents)
    ? allStudents.filter(Boolean)
    : [];
  ...
  const isStudentInClass = (student) => {
    ...
    return (
      ...
      safeExistingStudents.some((existing) => existing?._id === student?._id)
    );
  };

  // Filtrar alunos que ainda não estão na turma
  const availableStudents = safeAllStudents.filter(
    (student) => student && !isStudentInClass(student),
  );
    ...
    <div className="fixed inset-0 bg-black/75 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
          ...
          {page < totalPages && (
            <div className="mt-6 text-center">
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-lg border flex items-center justify-center mx-auto ${
                  loadingMore
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
          <div className="text-sm text-gray-600">
            Total de alunos carregados: {safeAllStudents.length}
          </div>
